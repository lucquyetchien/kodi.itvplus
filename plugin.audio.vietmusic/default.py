...
addon = xbmcaddon.Addon(id='plugin.audio.vietmusic')
...
def categories(url, mode): #1 lấy danh sách thể loại
  content = make_request('http://chiasenhac.vn')
    ...
    if href is not None:
      try:
        if href.startswith(url) and len(href) > len(url):
          ...
          add_dir(prefix + item.a.text, 'http://chiasenhac.vn/' + href, 2, logos+'vietmusic.png', query, type, 0)
      except:
        pass
  for item in items:
    href = item.a.get('href')
    if href is not None:
      try:
        if url != 'mp3/hot/' and url !='hd/video/' and href.startswith(url) and len(href) > len(url):
          add_dir('[COLOR blue]Albums[/COLOR]: ' + item.a.text, 'http://chiasenhac.vn/' + href + 'album.html', 3, logos+'vietmusic.png', query, type, 0)
      except:
        pass		
  return
  ...
  if url == '':
    ...
    for item in items:
      if item.a is not None:
        ...
        if href is not None:
          try:
            add_dir(item.a.text, 'http://chiasenhac.vn/' + href, 2, logos+'vietmusic.png', query, type, 0)
          except:
            pass			
    return
  # bảng xếp hạng
  if '/mp3/hot/' in url:  
    ...
    for table in tables:
      ...
      if item is not None:
        ...
        text = item.get('title')
        if text is not None:
          if 'chiasenhac.vn' not in href:
            href = 'http://chiasenhac.vn/' + href
...
def get_chiasenhac_album_songs(url = None): # lấy danh sách nhạc trong album
  ...
  for album in albums:
    a = album.findAll('a')
    if (len(a) == 3):
      href = 'http://chiasenhac.vn/' + a[1].get('href')
      text = album.text
      tt =text.split('-')
      if len(tt) == 2:
        text = tt[0] + ' - [COLOR firebrick]' + tt[1] + '[/COLOR]'
...
def search(query = '', page = 0, mode = 10, cat = 'music'): # tìm kiếm theo tên bài hát, ca sĩ, video clip
  ...
  url = 'http://search.chiasenhac.vn/search.php?s=' + urllib.quote(query) + '&page=' + str(page) + '&cat=' + cat
  if 'artist' in cat:
    url = 'http://search.chiasenhac.vn/search.php?s=' + urllib.quote(query) + '&page=' + str(page) + '&cat=music&mode=' + cat
...
def search_albums(start, query, page): # tìm kiếm theo album
  ...
  if page == 0:
    page = 1
  url = 'http://search.chiasenhac.vn/search.php?mode=album&s=' + urllib.quote_plus(query) + '&page=' + str(page) + '&start=' + start
  content = make_request(url)
  soup = BeautifulSoup(str(content), convertEntities=BeautifulSoup.HTML_ENTITIES)
...
def resolve_url(url):#100
    ...
    if 'http://' not in vid or 'mp3' in vid or 'mp4' in vid:
	    mediaUrl = urllib.unquote(vid).replace(' ','%20')
